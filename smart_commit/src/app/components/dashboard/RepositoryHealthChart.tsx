...
export default function RepositoryHealthChart() {
  ...
  const fetchRepositoryList = async () => {
    try {
      const response = await fetch(
        `/api/analytics/repository-health?days=${timeRange}&repository=all`
      );
      const result = await response.json();
      if (result.repositories?.length > 0) {
        setAvailableRepositories(result.repositories);
        // Set first repository as default
        setSelectedRepository(result.repositories[0]);
      }
    } catch (err) {
      console.error("Failed to fetch repository list:", err);
    }
  };
  ...
  const fetchHealthData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/analytics/repository-health?${params}`
      );
      if (!response.ok) throw new Error("Failed to fetch health data");

      const result = await response.json();
      console.log("Health API Response:", result);
      setData(result);

      // Store the full repository list only when fetching "all"
      if (selectedRepository === "all" && result.repositories?.length > 0) {
        setAvailableRepositories(result.repositories);
      }
    } catch (err) {
  ...
  const handleGenerateTestData = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        alert(
          `Generated ${result.commitsCreated} test commits! Refresh the page to see health metrics.`
        );
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 mb-4">
            No commits found in the analytics database.
          </p>
  ...
  const healthInfo = RepositoryHealthCalculator.getHealthLevel(
    data.healthMetrics.healthScore
  );
  const explanation = HealthExplainer.explainHealthScore(data.healthMetrics);
  const recommendations = HealthExplainer.getPriorityRecommendations(
    data.healthMetrics
  );
            ...
            <div className="flex items-center gap-2 mb-2">
              <div
                ...
                style={{
                  backgroundColor:
                    HEALTH_COLORS[
                      healthInfo.color as keyof typeof HEALTH_COLORS
                    ],
                }}
              ></div>
        ...
        <div className="w-full bg-gray-200 rounded-full h-3 mb-4">
          <div
            ...
            style={{
              ...
              backgroundColor:
                HEALTH_COLORS[healthInfo.color as keyof typeof HEALTH_COLORS],
            }}
          ></div>
        </div>
      </div>
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            <h4 className="text-md font-medium text-gray-800 mb-2">
              Why is your health score {data.healthMetrics.healthScore}/100?
            </h4>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-blue-50 p-4 rounded-lg relative group">
          <div className="flex items-center gap-2">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-1">Ideal: 20-100 lines</p>
            </div>
          </div>
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 w-64">
            ...
            <p>
              Commits in the 20-100 line range are easy to review and
              understand. Too small creates noise, too large is hard to review.
            </p>
          </div>
        </div>
        ...
        <div className="bg-green-50 p-4 rounded-lg relative group">
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 w-64">
            ...
            <p>
              Regular commits show consistent progress and reduce merge
              conflicts. Too few suggests infrequent development, too many may
              indicate rushed work.
            </p>
          </div>
        </div>
        ...
        <div className="bg-yellow-50 p-4 rounded-lg relative group">
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 w-64">
            ...
            <p>
              Focused commits touching few related files are easier to review
              and less likely to introduce bugs across multiple areas.
            </p>
          </div>
        </div>
        ...
        <div className="bg-red-50 p-4 rounded-lg relative group">
          ...
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 w-64">
            ...
            <p>
              Breaking changes disrupt users and require careful coordination.
              High frequency indicates unstable APIs or poor planning.
            </p>
          </div>
        </div>
      </div>
      ...
      {selectedRepository !== "all" && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          {data.commitSizeDistribution &&
            data.commitSizeDistribution.length > 0 && (
              <div>
                <h4 className="text-md font-medium text-gray-700 mb-3">
                  Commit Size Distribution
                </h4>
                  ...
                  {data.commitSizeDistribution.map((item, index) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          backgroundColor:
                            SIZE_COLORS[index % SIZE_COLORS.length],
                        }}
                      ></div>
                      <span className="text-gray-600">
                        {item.range}: {item.count} commits
                      </span>
                    </div>
                  ))}
                </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {data.repositories.map((repo) => {
              ...
              const repoHealthInfo = RepositoryHealthCalculator.getHealthLevel(
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      ...
                      style={{
                        backgroundColor:
                          HEALTH_COLORS[
                            repoHealthInfo.color as keyof typeof HEALTH_COLORS
                          ],
                      }}
                    >
