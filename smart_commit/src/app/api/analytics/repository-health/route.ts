...
} from "@/app/lib/health-calculator";
import {
  ...
  try {
    ...
    if (repository !== "all" && healthMetrics[repository]) {
      ...
      const commitSizeDistribution =
        calculateCommitSizeDistribution(repoCommits);
...
function calculateRepositoryHealth(
  ...
  const totalLinesChanged = commits.reduce(
    (sum: number, c: any) => sum + c.lines_added + c.lines_deleted,
    0
  );
  ...
  const codeStabilityScore = Math.max(
    0,
    100 - fileChurnRate * 10 - breakingChangeFrequency
  );
...
function calculateHealthTrends(commits: any[], days: number): HealthTrend[] {
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (dayCommits.length > 0) {
      const totalLines = dayCommits.reduce(
        (sum: number, c: any) => sum + c.lines_added + c.lines_deleted,
        0
      );
      const avgSize = totalLines / dayCommits.length;
      const totalFiles = dayCommits.reduce(
        (sum: number, c: any) => sum + (c.files_changed?.length || 0),
        0
      );
...
function calculateCommitSizeDistribution(
  ...
  commits.forEach((commit) => {
    ...
    const category =
      RepositoryHealthCalculator.categorizeCommitSize(totalLines);
